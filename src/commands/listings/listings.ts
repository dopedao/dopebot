...
export default {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand: SlashCommandSubcommandBuilder) =>
            subcommand.setName("mute")
                .setDescription("Mutes a certain dope from showing in listings for X days")
                ...
                .addNumberOption((option) => 
                    option.setName("days")
                    .setDescription("Amount of days you want to mute the dopeid")
                        .setMinValue(1)
                        .setMaxValue(10)
                        .setRequired(true)))
    ...
    async execute(interaction: CommandInteraction<CacheType>): Promise<void> {
        ...
        await fnMap[interaction.options.getSubcommand()](interaction, id, duration);
    }
}
...
const muteDope = async (interaction: CommandInteraction<CacheType>, dopeId: number, duration: number) => {
    if (mutedDopes.some(x => x.id == dopeId)) {
...
const unmuteDope = async (interaction: CommandInteraction<CacheType>, dopeId: number, duration: number) => {
    if (!mutedDopes.some(x => x.id == dopeId)) {
...
const viewMutedDopes = async (interaction: CommandInteraction<CacheType>, dopeId: number, duration: number) => {
