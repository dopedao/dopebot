...
import {
    getDailyMarketStatsEmbed,
    getWeeklyMarketStatsEmbed,
    getMonthlyStatsEmbed
} from '../../util/marketStatsEmbed';
import {
    SlashCommandBuilder,
...
const setContract = (type: string) => {
    return type == 'Hustler'
        ? Constants.HUSTLER_CONTRACT
        : Constants.GEAR_CONTRACT;
};
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            await fnMap[interaction.options.getSubcommand()!](
                interaction,
                interaction.options.getString('timeframe'),
                interaction.options.getSubcommand()[0].toUpperCase() +
                    interaction.options.getSubcommand().slice(1)
            );
        } catch (error: unknown) {
...
const getHustlerStats = async (
    interaction: CommandInteraction,
    timeFrame: string,
    type: string
): Promise<void> => {
    try {
        const qxHustlerStats = await sfetch<IMarketStats>(
            `${Constants.QX_API}/collection/${Constants.HUSTLER_CONTRACT}/stats`,
            { headers: { 'X-API-KEY': process.env.DBOT_QX_API_KEY } }
        );
...
const getGearStats = async (
    interaction: CommandInteraction,
    timeFrame: string,
    type: string
): Promise<void> => {
    try {
        const qxGearStats = await sfetch<IMarketStats>(
            `${Constants.QX_API}/collection/${Constants.GEAR_CONTRACT}/stats`,
            { headers: { 'X-API-KEY': process.env.DBOT_QX_API_KEY } }
        );
...
const sendDailyStatsEmbed = async (
    interaction: CommandInteraction,
    qxHustlerStats: IMarketStats,
    type: string
): Promise<void> => {
...
const sendWeeklyStatsEmbed = async (
    interaction: CommandInteraction,
    qxHustlerStats: IMarketStats,
    type: string
): Promise<void> => {
...
const sendMonthlyStatsEmbed = async (
    interaction: CommandInteraction,
    qxHustlerStats: IMarketStats,
    type: string
): Promise<void> => {
