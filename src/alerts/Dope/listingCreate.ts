...
import { AttachmentBuilder, Client, Colors, TextChannel } from 'discord.js';
        ...
        try {
            ...
            if (data.asset_events) {
                ...
                sortedEvents.forEach(async (sell: AssetEvent) => {
                    ...
                    const { dopeSVG, dopeRank, opened, claimed } =
                        await getParsedDope(
                            newSale.id,
                    ...
                    const openseaSellEmbed = new EmbedBuilder()
                        ...
                        .setTitle(
                            `⛵ Dope #${newSale.id} (Rank: ${dopeRank}) listed!`
                        )
                        .setColor(Colors.Green)
                        .setURL(`${Constants.OS_DOPE_LINK}/${newSale.id}`)
                        .setFields(
                            {
                                name: `🔹 ${(
                                    sell.starting_price /
